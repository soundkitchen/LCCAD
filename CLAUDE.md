...
- **ズーム 100% = 実寸**: ウィンドウがあるディスプレイの物理解像度から pt/mm を算出して 100% の基準とする（`DisplayDensityObserver` が `CanvasTransform.baselineScale` に供給）。起動時と ⌘0 (Actual Size) は実寸 100%。取得不能時のフォールバックは 1mm = 3pt。クランプは scale 0.5〜50 pt/mm
- **パン**: マウス中ボタンドラッグ
