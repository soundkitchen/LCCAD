...
- **ズーム直接入力**: 100% 部分が `TextField` で、任意の倍率を入力可能
  ...
  - `+` / `-` ボタン、Fit (⇧⌘0) と共存
- **Zoom to Fit (⇧⌘0)**: 表示中の全図形の外接矩形がビューに収まるようにズーム・パン（図形がなければ Actual Size と同じ動作）。ステータスバー右端の Fit ボタンも同じ
- **Actual Size (⌘0)**: 表示を実寸 100%（画面上の 1mm = 実物の 1mm）に戻し、原点をキャンバス中央に配置。100% の基準はウィンドウがあるディスプレイの物理解像度から算出（#62、起動時も実寸 100%）
...
| **無編集ガード** | 表示文字列が編集開始時から変わっていなければコミットしない（丸め表示 %.1f の書き戻しと no-op Undo を防止、#55） |
